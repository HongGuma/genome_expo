...
route.post('/save',(req,res)=>{
    try{
        ...
        const sql = `INSERT INTO company_registration (kor_name, eng_name, items, items_desc, kor_com_intro, eng_com_intro, kor_item_intro, eng_item_intro, contact, created_at) VALUES `+
        ...
        expoDB.query(sql, (err, result)=>{
            try {
                logger.info(`Apply for the event. input: ${company_reg.kor_name}/${company_reg.items}/ (ip${ip}) `);
                res.send('0');
            } catch(e){
                logger.err(e);
                res.send('-1');
