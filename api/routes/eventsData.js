...
const regDate = year + month + day + today.getHours().toString()+today.getMinutes().toString()+today.getSeconds().toString();
        ...
        expoDB.query(dupliSql,(err,result)=>{
            try {
                if(result != null && result.length > 0){
                    logger.info(`Duplicate apply. input:${spector.name}/${spector.phone} (ip:${ip})`);
                    res.send('-2'); // 참석자 정보 중복
                }else{
                    const sql = `INSERT INTO participants (name, phone, email, belong, event, genome_report_question, created_at) VALUES `+
                        `('${spector.name}','${phone}','${spector.email}','${spector.belong}','${txt}','${spector.question}','${regDate}');`;

                    expoDB.query(sql,(err,result)=>{
                        try {
                            logger.info(`Apply for the event. input: ${spector.name}/${phone}/${spector.email}/${spector.belong}/${spector.question}/${txt} (ip${ip}) `);
...
route.post('/load/checking',(req,res)=>{
    try{
                ...
                if(userData != null && userData.length > 0){
                    ...
                    if(tmp != null){
                        ...
                        expoDB.query("SELECT * FROM events",(err,eventData)=>{
                            ...
                            resultData.push({user_nm:tmp.name,user_ph:tmp.phone,user_email:tmp.email,
                                user_belong:tmp.belong,question:tmp.genome_report_question,events:eventList});
        ...
        //             logger.info(`request info. (ip: ${ip.slice(7,ip.length)}`);
        //             res.send('-2');
        //         }else{
        //             const joinData = data[0];
        //             const eventSql = `SELECT * FROM events`;
        //             expoDB.query(eventSql,(err,result)=>{
        //                 const events = new Array();
        //                 if(joinData.events != null && joinData.events != ''){
        //                     const part = joinData.events.split(',');
        //                     for(var p of part){
        //                         if(p == result[0].id){
        //                             events.push(p);
        //                         }
