...
route.post('/save/question',(req,res)=>{
    try{
        ...
        const sql = "INSERT INTO inquiries (ca_num,wr_name,wr_hp,title,is_pass,password,contents,ip_addr,read_cnt,created_at)" +
            "VALUES ('"+input_value.category+"','"+input_value.writer+"','"+input_value.phone+"','"+input_value.title+"'," +
            "'"+input_value.isPass+"',SHA('"+input_value.password+"'),'"+input_value.content+"','"+ip.slice(7,ip.length)+"'," +
            "0,'"+regDate+"')";
        // console.log(sql);
...
//     try {
//         fs.readFile('../data/images_crop/2018_expo/2018Expo_Investment_counseling_session/crop_consult.JPG',(err,data)=>{
//             if(err) {
//                 logger.error(err);
//                 res.send('-1')
//             }else{
